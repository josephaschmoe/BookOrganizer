...
All catalog data, briefing cache, and shelf definitions are stored in **Cloud Firestore** under each user's account (`users/{uid}/catalog/data`). Discussion guide text lives in `users/{uid}/briefings/{bookId}` (one doc per book). Audio metadata (voice, duration, status, per-variant) lives in `users/{uid}/briefingAudio/{bookId}`; the generated WAV files are stored in Firebase Storage under `users/{uid}/briefing-audio/`. Book photo metadata (type, caption, storage path) lives in `users/{uid}/bookPhotos/{bookId}`; the image files are stored in Firebase Storage under `users/{uid}/book-photos/{bookId}/`. This applies to both Google-authenticated and anonymous guest users. The active shelf selection is persisted to `localStorage` per user.
