...
- **Custom Sort Order** — Manually reorder books within a shelf using drag-and-drop (grab handle on each card) or move-up / move-down controls; custom order persists and is respected on shared shelf links
...
- **Quick Add vs Specific Edition** — Two cataloguing modes when adding a single book. **Quick Add** captures work-level metadata fast (title, author, original year); edition details are stripped unless an exact ISBN match makes them authoritative. **Specific Edition** is evidence-first: page photos and Perplexity enrichment are the primary bibliographic source; catalog/API matches are used for comparison and cover art only
- **Photo Book Lookup** — Photograph up to 3 images per book (cover, title page, copyright/publisher page recommended); user can designate which photo is the cover. Gemini extracts metadata, then searches Open Library and Google Books; multiple candidates shown with cover thumbnails; selecting an alternate swaps it to the top. In Specific Edition mode, non-ISBN flows can trigger Perplexity (`resolveEditionMetadata`) for richer edition enrichment
- **Metadata Refresh** — Re-run lookup on any existing book without creating a duplicate: reuse saved book photos (including the current cover), add new photos, or fall back to ISBN or manual search. Blank fields fill automatically; replacing existing bibliographic values requires explicit confirmation. If title or author changes, the book is flagged for manual briefing regeneration rather than auto-regenerating
...
- **Book Photos** — Attach additional photos to any book entry (beyond the cover): use the camera for a quick shot or pick multiple images from your gallery. Each photo is tagged by type (inscription, signature, title page, copyright page, illustration, binding/spine, condition, or a custom label) and can have a free-text caption. Photos display in a collapsible grid on the book detail panel and are viewable on shared shelf links. Browseable via prev/next arrows in the cover lightbox. A "Contains Addtl Photos" sort option filters the catalog to books with at least one extra photo. Photos are included in the full backup ZIP (schema v2)
...
- **Cover Search** — When adding or replacing a cover, choose to upload a photo or search online; search fetches covers from Open Library and Google Books and presents them as a thumbnail grid; tap any thumbnail to open a full lightbox preview with prev/next navigation; apply the cover with one tap; a "None of these? Open Google Images →" link appears when results are shown
...
- **Search Reviews** — Tap the Search Reviews button on any book's detail page to open a pre-populated web search for reviews of that title
...
- **Shelf & Book Sharing** — Generate public, read-only share links for any shelf or individual book; viewers need no account. Per-share controls: include personal notes (off by default), enable AI-powered Wikipedia lookup for viewers, and enable audio briefings for viewers. One active link per shelf or book at a time; revoke from the shelf/book settings or Account Settings at any time. All active links visible and manageable in Account Settings. The shared detail panel has an expand toggle for a wider reading view (state persists). Cover lightbox automatically attempts a higher-resolution Google Books image before falling back to the original
...
| `resolveEditionMetadata` — Perplexity edition enrichment (Specific Edition mode) | perplexity-sonar-pro |
...
All catalog data, briefing cache, and shelf definitions are stored in **Cloud Firestore** under each user's account (`users/{uid}/catalog/data`). Discussion guide text lives in `users/{uid}/briefings/{bookId}` (one doc per book). Audio metadata (voice, duration, status, per-variant) lives in `users/{uid}/briefingAudio/{bookId}`; the generated WAV files are stored in Firebase Storage under `users/{uid}/briefing-audio/`. Book photo metadata (type, caption, storage path) lives in `users/{uid}/bookPhotos/{bookId}`; the image files are stored in Firebase Storage under `users/{uid}/book-photos/{bookId}/`. Share records are stored in `users/{uid}/shares/{token}` (management layer); `shareLinks/{token}` provides public read access (compatibility/lookup layer). This applies to both Google-authenticated and anonymous guest users. The active shelf selection is persisted to `localStorage` per user.
