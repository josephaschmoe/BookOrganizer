...
- **Auto-Generated Briefings** — College-level discussion guides generated automatically in the background when books are added:
  ...
  - All briefings use **Perplexity Sonar Pro** with web-grounded verification and structured output
  - Supports three genre modes: **fiction** (spoiler/safe toggle, plot/characters/analysis/discussion questions), **non-fiction** (themes/key concepts/analysis/takeaways), and **reference** (editorial approach/contents overview/production notes/notable features/ideal-for)
...
- **Account Settings** — Export a full backup ZIP (catalog JSON + all cover images) or permanently delete your account and all associated data; the Backups section shows job history with download links for completed exports
...
| Function | Model |
|---|---|
| `generateBriefing` — all books | perplexity-sonar-pro |
